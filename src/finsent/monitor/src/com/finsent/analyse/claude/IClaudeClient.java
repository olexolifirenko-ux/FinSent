...
public interface IClaudeClient
{
     ...
     * and an optional output {@code schema} (a JSON Schema the response is constrained to via the
     * Messages API {@code output_config.format}; {@code null} for free-form text). Default: ignore the
     * extras and delegate, so test stubs need not implement it; the real client ({@link ClaudeClient})
     * overrides this to enable thinking / structured output on the request.
     */
    default String complete(String model, String prompt, int maxTokens, boolean thinking, JsonNode schema)
