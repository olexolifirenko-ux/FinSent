...
public class EntryPolicy_utest
{
    ...
    private static final EntryPolicy FULL = new EntryPolicy("high", "full", true);
    ...
    {
        EntryPolicy lowOk = new EntryPolicy("low", "full", true);
    ...
    {
        assertTrue(FULL.qualifiesFast(fast("bearish", "full")));
        assertFalse("reduced is below the 'full' min", FULL.qualifiesFast(fast("bearish", "reduced")));
        assertFalse(FULL.qualifiesFast(fast("bearish", "skip")));
    }

    @Test
    public void fastConvictionGateIsTunable()
    {
        EntryPolicy reducedOk = new EntryPolicy("high", "reduced", true);
        assertTrue(reducedOk.qualifiesFast(fast("bullish", "reduced")));
        assertFalse("skip never qualifies", reducedOk.qualifiesFast(fast("bullish", "skip")));
    }
    ...
    public void reversalExitTriggersOnAConfirmedOppositeForAMomentumPosition()
    {
        ...
        assertTrue("bullish full opposite a short", FULL.isReversalExit(fast("bullish", "full"), shortPos));
        assertTrue("even reduced opposite exits", FULL.isReversalExit(fast("bullish", "reduced"), shortPos));
    }
    ...
    {
        Position shortPos = momentum(Side.SHORT);
        assertFalse("skip is not confirmed", FULL.isReversalExit(fast("bullish", "skip"), shortPos));
        assertFalse("same side is not opposite", FULL.isReversalExit(fast("bearish", "full"), shortPos));
        assertFalse("a news position is left alone", FULL.isReversalExit(fast("bullish", "full"), news(Side.SHORT)));
        assertFalse("flat -> nothing to reverse", FULL.isReversalExit(fast("bullish", "full"), null));

        EntryPolicy off = new EntryPolicy("high", "full", false);
        assertFalse("reversal disabled", off.isReversalExit(fast("bullish", "full"), shortPos));
    }
